...
import com.fifthgen.trafficsim.gui.helpers.LevelOfServiceCalculator;
...
import static com.fifthgen.trafficsim.gui.controlpanels.LevelOfServiceControlPanel.DEFAULT_COLOR;
...
public final class Renderer {
    ...
    public synchronized void drawStaticObjects(BufferedImage image) {
        ...
        if (regions != null && map_.getReadyState()) {
            ...
            Font font = new Font("Serif", Font.PLAIN, 1024);
            g2d.setFont(font);
        ...
        if (LevelOfServiceCalculator.LOS) {
            HashMap<Street, Integer> vehiclesInStreets = new HashMap<>();
            for (int i = 0; i < regions.length; i++) {
                for (int j = 0; j < regions[i].length; j++) {
                    Vehicle[] vehicles = regions[i][j].getVehicleArray();
                    for (int k = 0; k < vehicles.length; k++) {
                        Street currentStreet = vehicles[k].getCurStreet();
                        Integer vehiculesList = vehiclesInStreets.get(currentStreet);
                        if (vehiculesList == null) {
                            vehiculesList = 1;
                            vehiclesInStreets.put(currentStreet, vehiculesList);
                        } else {
                            vehiclesInStreets.put(currentStreet, vehiculesList + 1);
                        }
                    }
                    ...
                    for (Street street : streets) {
                        g2d.setPaint(Color.BLACK);
                        g2d.setFont(new Font("SansSerif", Font.BOLD, 1024));
                        if (vehiclesInStreets.get(street) != null) {
                            g2d.drawString(vehiclesInStreets.get(street) + "", (street.getStartNode().getX() + street.getEndNode().getX()) / 2, (street.getStartNode().getY() + street.getEndNode().getY()) / 2);
                        }
                    }
                }
            }
